...
//  사용자 Settings(@AppStorage "keepScreenAwake")이 꺼져 있으면 강제하지 않는다.
//  배터리 보호를 위해 Pause/Done/Stop/백그라운드에서 반드시 해제한다.
//
