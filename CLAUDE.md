...
- **2026-08-13 · `.plusLighter` 합성에서 불투명도를 일반 알파처럼 잡아 화면이 백색으로 탐**
  - 증상: 부화 리빌 전체화면 섬광을 `peakOpacity = 0.92` + `.blendMode(.plusLighter)`로 넣었더니 **1.4초간 화면 전체가 완전 백색**. 픽셀아트가 통째로 사라짐.
  - 원인: `.plusLighter`는 덮는 게 아니라 **더한다**(`dst + src`). 어두운 배경(23,19,16) 위에서 0.92×흰색을 더하면 즉시 255로 포화된다. 게다가 방사 그라디언트의 흰 코어를 반경의 22%까지 깔아둬서 포화 영역이 화면 대부분이었다.
  - 교훈: 덧셈 합성에선 불투명도 체감이 곱절이다. **0.7 언저리에서 시작**하고, 흰 코어는 반경의 10% 이내로 좁힌 뒤 바깥은 채도 있는 색(앰버)으로 길게 끌어 "가운데만 타고 바깥은 발광"을 만든다. 그리고 **코드에서 눈대중하지 말고 시뮬 스크린샷의 평균 휘도로 확인**할 것.
  - 덤: 전체화면 섬광은 광과민성 유발 요인이라 `accessibilityReduceMotion`에서 세기를 낮춰야 한다(연출을 지우지 말고 자극만 낮춘다). 화면 흔들림도 같이 끈다.

- **2026-08-13 · 전체화면 이펙트를 화면 중앙에서 터뜨려 알과 어긋남**
